...
class CrossImageAttentionStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        if latents is not None:
            ...
            expected_batch_size = batch_size * num_images_per_prompt
            assert actual_batch_size == expected_batch_size, \
                f"latents batch size {actual_batch_size} != expected {expected_batch_size} (prompt count: {batch_size}, num_images_per_prompt: {num_images_per_prompt})"
            # If latents are provided with correct batch size, use them directly
        ...
        for t in op:
            ...
            # Use all latents, not just first 4
            if run_config is not None and run_config.style_image_paths is not None:
                # Multi-style mode: use all latents
                ...
                # For no-swap, compute for all latents (output + all styles)
                ...
                # Replace output prediction with no-swap version (keep styles as swap version)
                noise_pred_swap[0] = noise_pred_no_swap[0]
            else:
                # Original single/dual style mode
                noise_pred_swap = self.unet(
                ...
                noise_pred_no_swap = self.unet(
                    torch.cat([latent_model_input[0:1], latent_model_input[3:4]], dim=0),
                    t,
                    encoder_hidden_states=torch.cat([prompt_embeds[0:1],prompt_embeds[3:4]], dim=0),
                    cross_attention_kwargs={'perform_swap': False},
                    return_dict=False,
                )[0]
